...
export default function ASTRoomOnlyPage() {
  ...
  const sections = [
    ...
    { id: "tenant", title: "Tenant Details", icon: FileText },
    { id: "property", title: "Property Details", icon: FileText },
    { id: "tenancy", title: "Tenancy Terms", icon: PoundSterling },
    { id: "utilities", title: "Utilities & Services", icon: FileText },
    { id: "deposit", title: "Deposit Protection", icon: FileText },
  ...
  const addTenant = () => {
    setFormData(prev => ({
      ...prev,
      tenantNames: [...prev.tenantNames, ""],
      tenantEmails: [...prev.tenantEmails, ""],
    }));
  };

  const removeTenant = (index: number) => {
    if (formData.tenantNames.length > 1) {
      setFormData(prev => ({
        ...prev,
        tenantNames: prev.tenantNames.filter((_, i) => i !== index),
  ...
  const validateSection = (sectionId: string) => {
    ...
    switch (sectionId) {
      case "landlord":
        ...
        if (!formData.landlordAddress) newErrors.landlordAddress = "Landlord address is required";
        break;
      case "tenant":
        formData.tenantNames.forEach((name, index) => {
          if (!name) newErrors[`tenantName${index}`] = "Tenant name is required";
        });
        break;
      case "property":
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                ...
                />
                {errors.landlordName && (
                  <p className="mt-1 text-sm text-red-400">{errors.landlordName}</p>
                )}
              </div>
              ...
              <div className="md:col-span-2">
                ...
                {errors.landlordAddress && (
                  <p className="mt-1 text-sm text-red-400">{errors.landlordAddress}</p>
                )}
              </div>
        ...
        return (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={addTenant}
                className="rounded-lg bg-primary/20 border border-primary/30 px-4 py-2 text-primary hover:bg-primary/30 transition text-sm font-medium"
              >
                Add Tenant
              </button>
            </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      ...
                      <input
                        type="text"
                        value={name}
                        onChange={(e) => handleArrayInputChange("tenantNames", index, e.target.value)}
                        className="w-full rounded-lg border border-white/20 bg-surface/50 px-4 py-3 text-white placeholder-elegant-muted focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
                        placeholder="Enter tenant's full name"
                      />
                      {errors[`tenantName${index}`] && (
                        <p className="mt-1 text-sm text-red-400">{errors[`tenantName${index}`]}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="email"
                        value={formData.tenantEmails[index] || ""}
                        onChange={(e) => handleArrayInputChange("tenantEmails", index, e.target.value)}
                        className="w-full rounded-lg border border-white/20 bg-surface/50 px-4 py-3 text-white placeholder-elegant-muted focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
            ...
            <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-blue-200">
                  ...
                  <p>If there is more than one tenant, you are all jointly and separately liable for the obligations in this agreement.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-white/20 bg-surface/50 px-4 py-3 text-white placeholder-elegant-muted focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
                  placeholder="e.g., Front bedroom on first floor"
                />
                {errors.roomDescription && (
                  <p className="mt-1 text-sm text-red-400">{errors.roomDescription}</p>
                )}
              </div>
              ...
              <div>
                ...
                {errors.propertyAddress && (
                  <p className="mt-1 text-sm text-red-400">{errors.propertyAddress}</p>
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-lg border border-white/20 bg-surface/50 px-4 py-3 text-white placeholder-elegant-muted focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
                  placeholder="e.g., Kitchen, bathroom, living room, garden"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-white/20 bg-surface/50 px-4 py-3 text-white placeholder-elegant-muted focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
                  placeholder="e.g., One designated parking space, Street parking only, N/A"
                />
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                ...
                {errors.rentAmount && (
                  <p className="mt-1 text-sm text-red-400">{errors.rentAmount}</p>
                )}
              </div>
              ...
              <div>
                ...
                {errors.firstPaymentDate && (
                  <p className="mt-1 text-sm text-red-400">{errors.firstPaymentDate}</p>
                )}
              </div>
            ...
            <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/5 p-4">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-yellow-200">
                  <p className="font-medium mb-1">Council Tax Note</p>
                  <p>For room-only lets, the landlord is always liable for council tax (as of 1st December 2023).</p>
                </div>
              </div>
            </div>
          </div>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-white mb-2">
                  Lead Tenant (for multiple tenants)
                </label>
                <input
                  type="text"
                  ...
                  className="w-full rounded-lg border border-white/20 bg-surface/50 px-4 py-3 text-white placeholder-elegant-muted focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
                  placeholder="Name of lead tenant to manage deposit, or N/A for single tenant"
                />
              </div>
            </div>
            ...
            <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
              <div className="flex gap-3">
                <Info className="h-5 w-5 text-blue-400 flex-shrink-0 mt-0.5" />
                <div className="text-sm text-blue-200">
                  <p className="font-medium mb-1">Deposit Protection Requirements</p>
                  <p>The deposit must be protected in a government-approved scheme within 30 days of receipt and prescribed information must be provided to the tenant.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-white/10 bg-panel/50 p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2 text-sm">
                ...
                <div>
                  <p className="text-elegant-muted">Tenant(s):</p>
                  <p className="text-white">{formData.tenantNames.filter(Boolean).join(", ") || "Not specified"}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-elegant-muted">Room:</p>
                  <p className="text-white">{formData.roomDescription || "Not specified"}</p>
                </div>
                <div>
              ...
              <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
                <div className="flex gap-3">
                  ...
                  <div className="text-sm text-red-200">
                    ...
                    <p>This agreement contains legally binding obligations. Ensure you understand all terms before signing. Consider seeking legal advice if needed.</p>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-8 space-y-2">
              {sections.map((section, index) => {
                ...
                const isCompleted = false; // Add validation logic here
